...
- (id)init;
{
    ...
    bufferedFramebuffers = [NSMutableArray array];
...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex {
    if ([bufferedFramebuffers count] >= _bufferSize) {
        self.outputFramebuffer = bufferedFramebuffers[0];
        [bufferedFramebuffers removeObjectAtIndex:0];
    } else {
        // Nothing yet in the buffer, so don't process further until the buffer is full
        self.outputFramebuffer = [self getInputFramebuffer:0];
        [self.outputFramebuffer lock];
    }
    
    [bufferedFramebuffers addObject:self.outputFramebuffer];
    ...
    [self loopTargetsWithTargetAndTextureIndex:^(id<GPUImageInput> target, NSUInteger textureIndex) {
        [target setInputRotation:[self getInputRotation:0] index:textureIndex];
    }];
