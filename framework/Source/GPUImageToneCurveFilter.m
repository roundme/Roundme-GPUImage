...
- (id)init;
{
    ...
    toneCurveTextureUniform = [self.filterProgram uniformIndex:@"toneCurveTexture"];
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
...
- (NSArray *)getPreparedSplineCurve:(NSArray *)points
{
    if (points && [points count] > 0) 
    {
        ...
        for (int i=0; i<[splinePoints count]; i++) 
        {
            ...
            float distance = sqrtf(powf((origPoint.x - newPoint.x), 2.0f) + powf((origPoint.y - newPoint.y), 2.0f));
