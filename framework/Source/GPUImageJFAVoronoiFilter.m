...
- (id)init;
{
    ...
    sampleStepUniform = [self.filterProgram uniformIndex:@"sampleStep"];
    sizeUniform = [self.filterProgram uniformIndex:@"size"];
...
- (void)renderToTextureWithVertices:(const GLfloat *)vertices textureCoordinates:(const GLfloat *)textureCoordinates {
    ...
    glUniform1f(sampleStepUniform, 0.5f);
    
    glUniform2f(sizeUniform, _sizeInPixels.width, _sizeInPixels.height);

    glBindTexture(GL_TEXTURE_2D, [[self getInputFramebuffer:0] texture]);
...
//        glUniform1i(self.filterInputTextureUniform, 2);
        ...
        float step = powf(2.0f, numPasses - pass) / powf(2.0f, numPasses);
