...
- (void)yuvConversionSetup;
{
    if ([GPUImageContext supportsFastTextureUpload])
    {
        runSynchronouslyOnVideoProcessingQueue(^{
            ...
            isFullYUVRange       = YES;
            yuvConversionProgram = [[GLProgram alloc] initWithVertexShaderString:kGPUImageVertexShaderString fragmentShaderString:kGPUImageYUVFullRangeConversionForLAFragmentShaderString];
...
- (void)processAsset
{
    ...
    if (synchronizedMovieWriter != nil) {
        ...
        [synchronizedMovieWriter setVideoInputReadyCallback:^{
            ...
            return [self readNextVideoFrameFromOutput:readerVideoTrackOutput];
        }];

        [synchronizedMovieWriter setAudioInputReadyCallback:^{
            __strong typeof(self) self = weakSelf;
            return [self readNextAudioSampleFromOutput:readerAudioTrackOutput];
        }];
    ...
    if (reader.status == AVAssetReaderStatusReading && ! videoEncodingIsFinished)
    {
        ...
        if (sampleBufferRef) 
        {
            ...
            if (_playAtActualSpeed)
            {
                ...
                GLfloat frameTimeDifference = CMTimeGetSeconds(differenceFromLastFrame);
                GLfloat actualTimeDifference = currentActualTime - previousActualFrameTime;
...
- (void)processMovieFrame:(CVPixelBufferRef)movieFrame withSampleTime:(CMTime)currentSampleTime {
    ...
    if ([GPUImageContext supportsFastTextureUpload]) {
        ...
        if (CVPixelBufferGetPlaneCount(movieFrame) > 0) {
            ...
            NSAssert1(!err, @"Error at CVOpenGLESTextureCacheCreateTextureFromImage %d", err);
...
//            }
            ...
            [self loopTargetsWithTargetAndTextureIndex:^(id<GPUImageInput> target, NSUInteger textureIndex) {
                [target setInputSize:CGSizeMake(bufferWidth, bufferHeight) index:textureIndex];
            ...
            [self loopTargetsWithTargetAndTextureIndex:^(id<GPUImageInput> target, NSUInteger textureIndex) {
                [target newFrameReadyAtTime:currentSampleTime atIndex:textureIndex];
            }];
...
//            outputTexture = 0;
        ...
        self.outputFramebuffer = [[GPUImageContext sharedFramebufferCache] fetchFramebufferForSize:CGSizeMake(bufferWidth, bufferHeight) textureOptions:self.outputTextureOptions onlyTexture:YES];
...
- (void)convertYUVToRGBOutput {
    [GPUImageContext setActiveShaderProgram:yuvConversionProgram];
    self.outputFramebuffer = [[GPUImageContext sharedFramebufferCache] fetchFramebufferForSize:CGSizeMake(imageBufferWidth, imageBufferHeight) onlyTexture:NO];
