...
- (id)init
{
    ...
    hueAdjustUniform = [self.filterProgram uniformIndex:@"hueAdjust"];
    self.hue = 90;
    
    return self;
}

- (void)setHue:(GLfloat)newHue
{
    // Convert degrees to radians for hue rotation
    hue = fmodf(newHue, 360.0) * M_PI/180;
    [self setFloat:hue forUniform:hueAdjustUniform program:self.filterProgram];
}
