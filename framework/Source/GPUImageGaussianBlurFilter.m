...
+ (NSString *)fragmentShaderForStandardBlurOfRadius:(NSUInteger)blurRadius sigma:(GLfloat)sigma;
{
    ...
    {
        standardGaussianWeights[currentGaussianWeightIndex] = (1.0f / sqrtf(2.0f * M_PI * powf(sigma, 2.0f))) * expf(-powf(currentGaussianWeightIndex, 2.0f) / (2.0f * powf(sigma, 2.0f)));
...
- (void)switchToVertexShader:(NSString *)newVertexShader fragmentShader:(NSString *)newFragmentShader;
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        self.filterProgram = [[GLProgram alloc] initWithVertexShaderString:newVertexShader fragmentShaderString:newFragmentShader];
        ...
        [self setInputTextureCoordinateAttribute:[self.filterProgram attributeIndex:@"inputTextureCoordinate"] index:0];
        [self setInputTextureUniform:[self.filterProgram uniformIndex:@"inputImageTexture"] index:0];
        verticalPassTexelWidthOffsetUniform = [self.filterProgram uniformIndex:@"texelWidthOffset"];
        verticalPassTexelHeightOffsetUniform = [self.filterProgram uniformIndex:@"texelHeightOffset"];
        [GPUImageContext setActiveShaderProgram:self.filterProgram];

        glEnableVertexAttribArray(self.filterPositionAttribute);
        glEnableVertexAttribArray([self getInputTextureCoordinateAttribute:0]);

        secondFilterProgram = [[GLProgram alloc] initWithVertexShaderString:newVertexShader fragmentShaderString:newFragmentShader];
...
- (void)setTexelSpacingMultiplier:(GLfloat)newValue;
{
...
- (void)setBlurRadiusInPixels:(GLfloat)newValue;
{
    ...
    if (roundf(newValue) != _blurRadiusInPixels)
    {
        _blurRadiusInPixels = roundf(newValue); // For now, only do integral sigmas
        
        NSUInteger calculatedSampleRadius = 0;
        if (_blurRadiusInPixels >= 1) // Avoid a divide-by-zero error here
        {
            // Calculate the number of pixels to sample from by setting a bottom limit for the contribution of the outermost pixel
            GLfloat minimumWeightToFindEdgeOfSamplingArea = 1.0f / 256.0f;
            calculatedSampleRadius = floor(sqrtf(-2.0f * powf(_blurRadiusInPixels, 2.0f) * logf(minimumWeightToFindEdgeOfSamplingArea * sqrtf(2.0f * M_PI * powf(_blurRadiusInPixels, 2.0f))) ));
